...
11. No softBASIC call for nearest-neighbour (pixel-art) texture filtering. Surfaced by the same "collect the coins" platformer briefing as #9: scaled-up pixel art renders smoothed/blurry, and PIXI fixes the scale mode at texture-creation time so it can only be set before assets load. The *timing* half of this is now solved — `oninit` (shipped 2026-08-04) is a hook that runs before preloading — but the rendering option itself still has no API. Tracked as `docs/language/library-roadmap.md` P12; deliberately left out of the `oninit` change so the language mechanism and the demo-specific rendering option ship separately.
