...
| `src/constants/firstPartyPackages.ts` | MODIFY | Add `file`, `save` to `softcore`'s `moduleNames` (alongside `math`/`string`/`array`) — general-purpose utilities with no conceptual tie to graphics/game-runtime, unlike everything in `softgfx` (sprites, cameras, scenes, collision, audio, tilemaps). Needing `_sb` engine bridging isn't a reason to group with `softgfx`: `_sb` is assembled unconditionally in `softBasicEngine.js` regardless of which packages a project selects, so package placement is a conceptual grouping choice, not a technical requirement |
