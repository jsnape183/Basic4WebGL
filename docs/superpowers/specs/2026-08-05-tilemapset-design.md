...
**2026-08-05 — `tileAt` now accounts for ancestor transforms; `TileMapSet.tileAt(name, x, y)` convenience method added.** The limitation noted immediately above this line at the time of writing — `TileMapLayer.tileAt()` not accounting for a parent `TileMapSet.transform` offset — is resolved. Rather than special-casing `TileMapSet`, the shared `tileAt(handle, worldX, worldY)` engine function (used by `TileMap`, `TileMapLayer`, and now `TileMapSet.tileAt`) walks `handle.parent` summing each ancestor's local `x`/`y`, stopping at (and excluding) `worldContainer`/`hudContainer` — so camera pan is still correctly excluded from the result, matching the original single-layer contract exactly, while any nesting depth (including a `TileMapLayer` inside a `TileMapSet`) is now handled generically. A plain `TileMap`/`TileMapLayer` with no set involved is unaffected (its only ancestor is `worldContainer`, so the loop terminates after one step, identical to the old direct `handle.x`/`handle.y` read). `TileMapSet` also gained a `tileAt(name, x, y)` convenience method — `_sb.tileAtInSet(setHandle, name, worldX, worldY)` looks up the layer via the existing `getTileMapSetLayer` and delegates to `tileAt`, so there is exactly one offset-accumulation implementation. Engine tests: `tests/components/Runner/tilemap.test.ts` (the first dedicated unit test for `engine/tilemap.js`, following the `camera.test.ts`/`lifecycle.test.ts` "evaluate the raw script in a Function context" pattern). Transpiler tests: `tests/lib/Basic4WebGL/unit/transpiler/tilemapset.test.ts`. Docs: `src/docs/api-reference/tilemapset.md`.
