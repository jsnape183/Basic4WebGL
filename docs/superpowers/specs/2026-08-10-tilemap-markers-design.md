...
- **All tile layers in one document are assumed to share the same grid dimensions** — the editor derives the marker grid's `rows`/`cols` from the *first* tile layer in the document, since a marker layer has no dense array of its own to size itself from. The editor's own UI (`handleAddLayer`, `NewTilemapDialog`) always keeps every tile layer uniformly sized, so this is unreachable through normal use. A hand-edited or externally-generated `.stm` file with mismatched tile-layer dimensions is unsupported — no validation or error, `MarkerCanvas` just renders sized to whichever tile layer happens to be first, silently misaligned relative to any differently-sized layer. Flagged during Task 9's code review; not built at that time since it's not reachable via the editor itself, not because it's out of scope to ever address.
