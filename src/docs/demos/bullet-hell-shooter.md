...
Each level (`Level1Scene`, `Level2Scene`, `Level3Scene`) loads its own `.stm` tilemap, spawns the player, and builds its spawn points and weapon pickups from **tagged markers** placed visually in the Tilemap Editor — `tileMapSet.markersByTag("spawn")` and `tileMapSet.markersByTag("pickup")` — instead of hardcoding coordinates in the scene's own `.bas` file. That logic, along with the shared HUD setup, the per-frame HUD refresh, and the level-clear/game-over checks, lives in **`LevelHelpers.bas`**, a plain module (no `Class` keyword) called from each scene as `levelhelpers.someFunction(...)`.
...
Two real transpiler bugs were hit and worked around during development, documented inline as comments in `Mob.bas` and `Bullet.bas`:

- Typing a **Constructor parameter** with `as sometype` (e.g. `Constructor(x, y, targetRef as sprite)`) compiles without error but emits invalid JavaScript — the parameter name collides with the reserved word `this`/`constructor` handling. The fix is to leave constructor parameters untyped and instead type the **field** they get assigned to (e.g. `dim chaseTarget as sprite`), which is enough for dotted member access later.
- Declaring a **Constructor-scope local** with `dim` and using it across multiple statements in the constructor body (e.g. a shared `speed` local reused in several `if` branches) compiles clean but emits invalid JavaScript — a bare, undeclared assignment followed by `constructor.speed`, i.e. the literal word `constructor` used as an object, on every later read. This throws `constructor_speed is not defined` the instant the object is constructed. The fix is to avoid a shared constructor-scope local entirely and set `self.<field>` directly in each branch instead.
