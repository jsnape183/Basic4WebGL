...
' Read-only accessors onto the static-light arrays bakeStatic() fills. Any
' renderer that projects lights to screen space (e.g. an overlay pass) needs
' to enumerate each static light's position/height/intensity/radius. Additive.
