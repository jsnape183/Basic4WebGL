...
**Spacebar** fires — hit detection checks every enemy in the current level and picks the closest one whose projected screen position is within 15 columns of the centre ray and confirmed visible by the z-buffer. Health carries over between levels rather than resetting, so death only ever comes from cumulative damage across a run — every hit briefly flashes the screen red, and the current level is always shown in the HUD. Dying switches to a real game-over screen showing the level reached and a **persisted best level**, carried across page reloads via \`save\`.
